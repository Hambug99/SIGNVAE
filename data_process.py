...
# =============== 5. 显示 OpenPose 骨架动画（遍历数据） ===============
def visualize_motion_sequence(sequence, output_base_path, sample_id, mode):
    """
    以视频的方式展示 OpenPose 估计的骨架序列，并保存帧到 output_path
    :param sequence: (num_frames, num_points, 2) 关键点数据
    :param output_base_path: 总的输出目录
    :param sample_id: 当前样本的索引
    :param mode: "gt" 或 "pred"，用于区分是 Ground Truth 还是 预测结果
    """
    output_path = os.path.join(output_base_path, f"sample_{sample_id:03d}", mode)
    os.makedirs(output_path, exist_ok=True)  # 创建输出目录
    ...
    for index, (pose_keypoints, hand_left_keypoints, hand_right_keypoints, face_keypoints) in enumerate(
            zip(pose_keypoints_list, hand_left_keypoints_list, hand_right_keypoints_list, face_keypoints_list)):
        fig, ax = plt.subplots(figsize=(6, 4))
        ...
        for start, end in skeleton:
            ...
            b = np.all(pose_keypoints[end] < 20)
            if a or b:
                continue  # 如果关键点坐标异常，则跳过此连接
            ax.plot(
                [pose_keypoints[start, 0], pose_keypoints[end, 0]],
                [-pose_keypoints[start, 1], -pose_keypoints[end, 1]],  # y坐标取反以匹配图像坐标系统
                c='#7898e1'  # 蓝色骨骼
            )
        ...
        plt.close(fig)
        image = Image.fromarray(image)
        image.save(f"{output_path}/frame_{index:03d}.png")
